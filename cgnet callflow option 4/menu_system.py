...
def mainMenu():
    ...
    keyDict['4'] = (playBackSelf,('skip-post-1',))
    ...
    try:
        ...
        for i in range(1,3):
            ...
            playFile(PROMPTS_DIR+'option4', keyDict)
...
def playBack(intro=None):
    ...
    keyDict2['4'] = (playBackSelf,('skip-post-1',))
    ...
    for i in range(1,4):
        ...
        playFile(PROMPTS_DIR+'impact-3', keyDict2)
        playFile(PROMPTS_DIR+'option4', keyDict2)
...
def playBackSelf(intro=None):
    keyDict = newKeyDict()
    cdr_id = db.getcallIDByUser(user)
    countKeyPress(4, cdr_id)
    global tendigit
    tendigit=10
    ...
    playFile(PROMPTS_DIR+'monu', keyDict)
    while(tendigit > 0):
        playFile(PROMPTS_DIR+'wait-5-seconds', keyDict)
    customUser=entered_code
    # temporary fix so that incoming calls have a user
    keyDict = newKeyDict()
    posts = db.getCustomPostsInChannel('12345', swaraChannel, customUser)
    if len(posts) == 0:
        return playFile(PROMPTS_DIR+'no-comments', keyDict)
    playFile(PROMPTS_DIR+'mistake-0', keyDict)
    playFile(PROMPTS_DIR+intro, keyDict)
    ...
    for postID in posts:
        ...
        db.addCustomPlaybackEvent(postID, tpb, customUser) #ARJUN PATCHED with CallID
    tpbm = stopwatch.Timer()
    playFile(PROMPTS_DIR+'for-older-posts')
    keyDict2 = newKeyDict()
    keyDict2['1'] = (addComment,())
...
def addComment():
    ...
    try:
        ...
        # outside of the training channel, only keep audio files that are longer than 5 seconds
        if (swaraChannel=="training" or float(audio.info.length) >= 5):
            newCommentID = db.addCommentToChannel(user, auth, '12345', swaraChannel)
            #newCommentID = db.addCommentToChannel(user, '12345')
            debugPrint("moving " + AST_SOUND_DIR+commentTempFileName+".mp3" + " to " + SOUND_DIR+"web/"+str(newCommentID)+".mp3");
            shutil.move(AST_SOUND_DIR+commentTempFileName+".mp3", SOUND_DIR+"web/"+str(newCommentID)+".mp3")
            debugPrint("moving " + AST_SOUND_DIR+commentTempFileName+".wav" + " to " + SOUND_DIR+str(newCommentID)+".wav");
            shutil.move(AST_SOUND_DIR+commentTempFileName+".wav", SOUND_DIR+str(newCommentID)+".wav")
